							...
							const char *server)
{
	...
	connman_info("Add DNS server %s domain %s interface %s",
...
int connman_resolver_remove_all(const char *interface)
{
	...
	_DBG_RESOLVER("interface %s", interface);
							...
							const char *server)
{
	...
	_DBG_RESOLVER("interface %s server %s", interface, server);
