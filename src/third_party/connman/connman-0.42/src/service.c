 ...
 *  1. Prefer ready service to !ready
 *  2. Prefer wired Ethernet to Bluetooth to WiFi to WiMax to Cellular
...
static gint service_compare(gconstpointer a, gconstpointer b,
							gpointer user_data)
{
#define	PREFER_SERVICE(a,b,s) do { \
	if ((a)->type == (s)) return -1; \
	if ((b)->type == (s)) return 1; \
} while (0)
	...
	if (service_a->type != service_b->type) {
		 ...
		 * equally well
		 */
		PREFER_SERVICE(service_a, service_b,
					...
					enum connman_service_state state)
{
	...
	_DBG_SERVICE("service %p state %d", service, state);
