...
/* Chromium OS addition, sleffler, 9/25/2009: add support for -I option */
					...
					struct connman_element *element)
{
	...
	/* XXX DBG_ANY? */
	if (__connman_debug_enabled(DBG_ELEMENT) == FALSE)
		return;
				...
				const char *name, int type, const void *value)
{
	...
	_DBG_ELEMENT("name %s type %d value %p", name, type, value);
...
gboolean __connman_device_isfiltered(const char *devname)
{
	if (device_filter != NULL &&
		...
		_DBG_ELEMENT("ignoring device %s (no match)", devname);
		return TRUE;
	}
	if (nodevice_filter != NULL &&
	    g_pattern_match_simple(nodevice_filter, devname) == TRUE) {
		_DBG_ELEMENT("ignoring device %s (match no)", devname);
		return TRUE;
	}
	return FALSE;
}
