...
#define	_DBG_SUPPLICANT(fmt, arg...)	DBG(DBG_WIFI, fmt, ## arg)
...
static int task_connect(struct supplicant_task *task)
{
	...
	_DBG_SUPPLICANT("address %s security %s passphrase %s",
...
static void supplicant_probe(DBusConnection *conn, void *user_data)
{
	...
	for (list = driver_list; list; list = list->next) {
		...
		_DBG_SUPPLICANT("driver %p name %s", driver, driver->name);
