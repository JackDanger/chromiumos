...
#define	_DBG_UDHCP(fmt, arg...)	DBG(DBG_UDHCP, fmt, ## arg)
...
static int udhcp_probe(struct connman_element *element)
{
	...
	_DBG_UDHCP("element %p name %s", element, element->name);
...
static void udhcp_bound(DBusMessage *msg, gboolean renew)
{
	...
	_DBG_UDHCP("%s ==> address %s gateway %s", interface, address, gateway);
