...
#define	_DBG_DNSPROXY(fmt, arg...)	DBG(DBG_DNSPROXY, fmt, ## arg)
							...
							gpointer user_data)
{
	...
	_DBG_DNSPROXY("id 0x%04x rcode %d", hdr->id, hdr->rcode);
					...
					const char *domain, const char *server)
{
	...
	/* NB: we depend on host routes being installed for each dns server */
...
{
	_DBG_DNSPROXY("interface %s server %s", data->interface, data->server);
					...
					char *name, unsigned int size)
{
	...
	_DBG_DNSPROXY("id 0x%04x qr %d opcode %d qdcount %d",
	...
	_DBG_DNSPROXY("query %s (%d labels)", name, label_count);
				...
				const struct sockaddr *to, socklen_t tolen)
{
	...
	_DBG_DNSPROXY("id 0x%04x qr %d opcode %d", hdr->id, hdr->qr, hdr->opcode);
				...
				const char *query, const char *domain)
{
	...
	_DBG_DNSPROXY("query %s domain %s", query, domain);
...
static gboolean request_timeout(gpointer user_data)
{
	...
	_DBG_DNSPROXY("id 0x%04x", req->srcid);
							...
							gpointer user_data)
{
	...
	if (err < 0 || g_slist_length(server_list) == 0) {
		_DBG_DNSPROXY("FAIL: err %d server_list length %d", err,
	...
	if (req == NULL) {
		_DBG_DNSPROXY("no memory for request");
		return TRUE;
	}
...
static void destroy_listener(void)
{
	...
	for (list = request_list; list; list = list->next) {
		...
		_DBG_DNSPROXY("Dropping request (id 0x%04x -> 0x%04x)",
						req->srcid, req->dstid);
