...
#define	_DBG_HSO(fmt, arg...)	_DBG_MODEM(fmt, ## arg)
...
static void owandata_callback(const char *buf, void *user_data)
{
	...
	if (err != 8) {
		_DBG_HSO("parsed %d arguments", err);
		return;
	}

	_DBG_HSO("ip %s dns %s %s val %s", ip, ns1, ns2, val);
