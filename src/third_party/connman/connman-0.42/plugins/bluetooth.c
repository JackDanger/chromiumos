...
#define	_DBG_BLUETOOTH(fmt, arg...)	DBG(DBG_BLUETOOTH, fmt, ## arg)
...
{
	_DBG_BLUETOOTH("network %p", network);
...
static void connect_reply(DBusPendingCall *call, void *user_data)
{
	...
	_DBG_BLUETOOTH("interface %s", interface);
...
{
	_DBG_BLUETOOTH("device %p", device);
...
static void bluetooth_connect(DBusConnection *connection, void *user_data)
{
	...
	_DBG_BLUETOOTH("connection %p", connection);
