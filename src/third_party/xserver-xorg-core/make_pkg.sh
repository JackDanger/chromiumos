...
# TODO: Push the LDFLAGS into a patch along with other custom X Server
# configuration. The -lcrypto is currently needed for the xkbcomp patch.
pushd "$PKG_BUILD_DIR"/build
dch -i "ChromeOS Patches"
LDFLAGS=-lcrypto dpkg-buildpackage -j$NUM_JOBS -b -tc -us -uc
