        ...
        case XK_KP_Enter:
            if (field==Get_Name){
                ...
                if (NameBuffer==CONSOLE_STR){
                    ...
                    if (string::npos == NameBuffer.find('@')) {
#if defined(DEFAULT_LOGIN_DOMAIN)
                        NameBuffer.append(DEFAULT_LOGIN_DOMAIN);
#else
                        NameBuffer.append("@gmail.com");
#endif
                    }
                    action = Login;
                }
...
void Panel::ShowText(){
    ...
    XftDraw *draw = XftDrawCreate(Dpy, Win,
                                  DefaultVisual(Dpy, Scr), DefaultColormap(Dpy, Scr));
    /* welcome message */
    /* sosa@chromium.org - dynamically obtain version information and
     * use it instead of other welcome message */
    ifstream release_file;
    release_file.open("/etc/lsb-release", ios::in);
    if (release_file.is_open()) {
        string current_line;
        while (getline(release_file, current_line)) {
            if (current_line.find("RELEASE_DESCRIPTION") != string::npos) {
                welcome_message = current_line.substr(
                    current_line.find_last_of('=') + 1);
                break;
            }
        }
        release_file.close();
    } else {
        cerr << "Slim could not read the release description file"
             << endl;
    }
